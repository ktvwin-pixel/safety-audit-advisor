...
앱 화면의 `앱 설치` 버튼은 브라우저가 PWA 설치를 지원하면 설치 절차를 실행합니다. 설치 프롬프트가 제공되지 않는 PC에서는 같은 영역의 `설치 파일 다운로드`를 사용하여 `safety-audit-advisor-portable.zip`을 내려받고, 압축 해제 후 실행하거나 기관 홈페이지 서버에 업로드할 수 있습니다.
...
기관명 칸을 비워둔 상태에서 공식 홈페이지 URL을 입력하면, 중앙부처 및 주요 지방정부 도메인 기준으로 기관명을 자동 보완합니다. 자동 보완이 어려운 도메인은 도메인 식별명을 임시 기관명으로 표시하므로, 필요 시 담당자가 기관명 칸을 직접 수정하면 됩니다.

복지시설 화재 대응 인터뷰, 시설물 사고 면담 질문지, 무응답 대비 보완통지서와 같은 전문 산출물은 백데이터로 내장되어 있으나 모든 답변에 의무적으로 붙지 않습니다. 현재 입력 질문에 해당 분야 키워드가 포함된 경우에만 관련 모듈을 출력합니다.
